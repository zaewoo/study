...
## Branch
로컬 레포지토리에 두 개 이상의 브랜치를 둘 수 있는데, 일반적으로 한 개의 브랜치를 개발 브랜치, 다른 한 개의 브랜치를 운영 브랜치로 둔다.\
그리고 변경 사항은 모두 로컬 레포지토리의 개발 브랜치에 커밋을 하여 정보를 반영한다.\
다음으로 이 변경 사항을 테스트하기 위해 리모트 레포지토리의 개발 브랜치에 푸시하고, 리모트 레포지토리에서 테스트를 한다.\
이상이 없을 시 로컬 레포지토리의 개발 브랜치 정보를 모두 운영 브랜치에 머지하고, 리모트 레포지토리의 운영 레포지토리에 배포한다.

## 배포
* git branch: 현재 브랜치 확인
* git remote add origin https://github.com/zaewoo/airflow.git: 원격 레포지토리 연결
* git remote: 원격 레포지토리 확인
* git push -u origin master: 원격 레포지토리 배포
